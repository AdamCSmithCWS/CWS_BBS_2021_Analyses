                         ...
                         ...)
{
  ...
  routes_df <- unique(spsp_f[,c("site","route","strat","strat_name")])
  routes_df <- routes_df[order(routes_df$strat,routes_df$site),]
    
  nroutes_strata <- routes_df %>% arrange(strat,
                                          site) %>% 
    group_by(strat) %>% 
    summarise(nsites = n())
  
  nroutes_st <- tapply(routes_df$site,routes_df$strat,length)
  nroutes_strata <- data.frame(strat = as.integer(names(nroutes_st)),nsites = as.integer(nroutes_st))
  
  
  nsites_strata <- as.integer(nroutes_strata$nsites)
  maxnsites_strata <- max(nsites_strata)
  nstrata <- max(nroutes_strata$strat)
  ste_mat <- matrix(data = 0,
                    nrow = nstrata,
                    ncol = maxnsites_strata)
  for(i in 1:nstrata){
    ste_mat[i,1:nsites_strata[i]] <- routes_df[which(routes_df$strat == i),"site"]
  }
