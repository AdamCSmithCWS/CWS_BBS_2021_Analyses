...
#' @param stratify_by stratification used in the analysis
#' @param backend text string, either "JAGS" or "Stan"
                             ...
                             alt_region_names = NULL)
{
  ...
  {
    if(is.null(jags_mod$stratify_by))
    {
      stop("No stratification supplied to generate_indices(). If your fitted model object does not have an element called stratify_by, then supply one of bbs_usgs, bbs_cws, latlong, state, or bcr"); return(NULL)
    }

  }
  if(!is.null(jags_data)){
    data_list <- extract_index_data(jags_mod = jags_mod,alt_n = alternate_n,jags_data = jags_data,
                                    stratify_by = stratify_by, backend = backend)
  }else{
