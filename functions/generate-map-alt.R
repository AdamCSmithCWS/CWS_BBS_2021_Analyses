                         ...
                         min_backcast = 0.75)
{
  ...
  if(!is.null(heat_column)){
    if(!heat_column %in% names(trend)){  stop("heat_column must be a column of trend information in the trend data."); return(NULL)
    }
  }
  ...
  if(slope){
    ...
    if(!is.null(heat_column)){
      trend$Tplot <- as.numeric(as.character(trend[,heat_column]))
    }else{
  ...
  if(alpha_backcast){
    trend$alpha_b <- trend$backcast_flag
  }else{
    trend$alpha_b <- 1
  }
  if(exclude_backcast){
    trend <- trend %>% 
      filter(backcast_flag >= min_backcast)
  }
  ...
  mp.plot = ggplot2::ggplot()+
    ggplot2::geom_sf(data = map,ggplot2::aes(fill = Tplot,
                                             alpha = alpha_b),
    ...
    ggplot2::scale_alpha_continuous(range = c(0.7,1))+
    ...
    if(exclude_backcast){
      mp.plot <- mp.plot +
        ggplot2::labs(subtitle = paste("Only regions with data for >",min_backcast*100,"% of the trend-period are shown"))
    }
  return(mp.plot)
}
