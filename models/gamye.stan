...
// intercepts and slopes
  ...
  for(i in 1:ncounts){
    ...
    E[i] =  year_pred[year[i],strat[i]] + strata[strat[i]] + yeareffect[strat[i],year[i]] + rte[route[i]] + obs + noise;
  }
...
for(y in 1:nyears){
  ...
  real nsmooth_t_alt[nroutes_strata[s]];
      ...
      nsmooth_t_alt[t] = exp(strata[s] + year_pred[y,s] + rte[rte_mat[s,t]] + retrans_noise + retrans_obs);
