...
transformed parameters { 
  ...
  matrix[nyears,nstrata] year_pred_raw;
  ...
  for(s in 1:nstrata){
    beta[s,] = (sdbeta[s] * beta_raw[s,]);
  }
  Y_pred = year_basispred * BETA; 
  
      for(s in 1:nstrata){
     year_pred_raw[,s] = year_basispred * transpose(beta[s,]);
     year_pred[,s] = Y_pred + year_pred_raw[,s];
}
...
for(s in 1:nstrata){
  ...
  sum(beta_raw[s,]) ~ normal(0,0.001*nstrata);
}
