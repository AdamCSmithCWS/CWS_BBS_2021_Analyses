...
parameters {
 ...
 vector[nstrata] strata_raw;
  ...
  real<lower=0.000001> sdnoise;    // sd of over-dispersion  #lower bound is relevant to keep phi estimates < 1000
...
model {
  ...
  //modified priors in this _alt_sdbeta version of the model
  sdnoise ~ gamma(2,1); //~ student_t(3,0,1); //prior on scale of extra Poisson log-normal variance or inverse sqrt(phi) for negative binomial
  //end modified priors
  ...
  sdste ~ student_t(20,0,1); //prior on sd of site effects
  sdyear ~ gamma(2,4); // prior on sd of yeareffects - stratum specific, and boundary-avoiding with a prior mode at 0.25 ((2-1)/4) - recommended by https://doi.org/10.1007/s11336-013-9328-2 
  
  //modified priors in this _alt_sdbeta version of the model
  sdBETA ~ student_t(4,0,1); // prior on sd of GAM parameters lighter tails
  sdbeta ~ student_t(100,0,1); // prior on sd of GAM parameters almost normal tails
  
  sdstrata ~ student_t(20,0,1); //prior on sd of intercept variation
   //end modified priors
