...
three_gens <- read.csv("alt_Data/Generation_lengths_all_bbs_trend_species.csv") %>% 
  select(-english)
...
# species specific short-term trend ---------------------------------------

        #round three generations to nearest integer
        gen3 <- round(3*three_gens[which(three_gens$aou == aou),"GenLength"])
          # minimum three generations or 10-years
          # maximum of three generations or full time-series (in case some species three generation time is greater than full time-series)
          short_time <- min((YYYY-fy),max(c(10,gen3)))
        ...
        write.csv(tall,file = paste0("trends/three_generation/",species_f_bil,"_trends.csv"),
                  row.names = FALSE)
        ...
        pdf(file = paste0("trends/Trend_maps_three_generation/",species_f_bil,"_trend_maps.pdf"),
            width = 11,
            height = 8.5)
        print(map)
        print(map_25 / map_75)
        ...
        saveRDS(traj_out,file = paste0(output_dir,"/temp_rds_storage/",species_f,"3Gen_trend_maps.RDS"))
...
pdf(file = paste0("Figures/BBS_High_level_summary_3_Generation_",YYYY,".pdf"),
    height = 9,
    width = 17)
...
tmaps <- readRDS(paste0(output_dir,"/temp_rds_storage/",species_f,"3Gen_trend_maps.RDS"))
